...
class GenerateBlocAction : AnAction(), GenerateBlocDialog.Listener {
    ...
    override fun onGenerateBlocClicked(blocName: String?, shouldUseEquatable: Boolean,shouldUseLoadService: Boolean) {
        blocName?.let { name ->
            val generators = BlocGeneratorFactory.getBlocGenerators(name, shouldUseEquatable,shouldUseLoadService)
            generate(generators)
        }
    }
